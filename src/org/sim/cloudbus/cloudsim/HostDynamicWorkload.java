...
public class HostDynamicWorkload extends Host {
	...
	/** The state history. */
	private final List<HostStateHistoryEntry> stateHistoryForCpu = new LinkedList<HostStateHistoryEntry>();

	private final List<HostStateHistoryEntry> stateHistoryForRam = new LinkedList<HostStateHistoryEntry>();
	...
	public double updateVmsProcessing(double currentTime) {
		...
		addStateHistoryEntryForCpu(
				currentTime,
				getUtilizationOfCpu(),
				getUtilizationOfCpu(),
				(getUtilizationOfCpu() > 0));


		addStateHistoryEntryForRam(
				currentTime,
				getUtilizationOfRam(),
				getUtilizationOfRam(),
				(getUtilizationOfRam() > 0));
	 ...
	 */
	public List<HostStateHistoryEntry> getStateHistoryForCpu() {
		return stateHistoryForCpu;
	}

	public List<HostStateHistoryEntry> getStateHistoryForRam() {
		return stateHistoryForRam;
	}
	...
	public
			void
			addStateHistoryEntryForCpu(double time, double allocatedMips, double requestedMips, boolean isActive) {
		...
		HostStateHistoryEntry newState = new HostStateHistoryEntry(
				...
				requestedMips,
				isActive);
		if (!getStateHistoryForCpu().isEmpty()) {
			HostStateHistoryEntry previousState = getStateHistoryForCpu().get(getStateHistoryForCpu().size() - 1);
			if (previousState.getTime() == time) {
				getStateHistoryForCpu().set(getStateHistoryForCpu().size() - 1, newState);
				return;
			}
		}
		getStateHistoryForCpu().add(newState);
	}
	public
	void
	addStateHistoryEntryForRam(double time, double allocatedMips, double requestedMips, boolean isActive) {
		...
		HostStateHistoryEntry newState = new HostStateHistoryEntry(
				...
				requestedMips,
				isActive);
		if (!getStateHistoryForRam().isEmpty()) {
			HostStateHistoryEntry previousState = getStateHistoryForRam().get(getStateHistoryForRam().size() - 1);
			if (previousState.getTime() == time) {
				getStateHistoryForRam().set(getStateHistoryForRam().size() - 1, newState);
				return;
			}
		}
		getStateHistoryForRam().add(newState);
	}
